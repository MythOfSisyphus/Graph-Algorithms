...
(weight, node) pair
In this algo what we do is start with a given node then we visit its neighbors and add
them in priority queue then in the next iteration we got the node with minimum edge weight
and we visit its neighbors that's not in MST. And while visiting neighbors we
...
0  |  /  |  \  |  7
 \ | /   | / \ | /
  \|/    |/   \|/
   4-----5-----6
Now we start our MST with node 0, and we put (edge_weight, node) tuples (1, 1) and (2, 4) in our queue 
so (1, 1) will be popped up in the next iteration as 1 < 2. And node 1 has edge with nodes 0, 2, 4 but 
node 0 is already in MST so skip that and edge weight of node 4 with node 1 is 3 so again skip that because
we have shorter edge weight already in queue, and add (4, 2).
...
def prims_Algorithm(graph, star_node):
    ...
    while myheap:
        ...
        for neighbor, edge_weight in graph[current_node]:
            # update node_distance and neighbor if shorter distance is found.
